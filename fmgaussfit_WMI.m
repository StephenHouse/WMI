function [fitresult, zfit, fiterr, zerr, resnorm, rr] = fmgaussfit_EDIT(xx,yy,zz,xo,yo)
...
%% This version has been modified to work better with my (Stephen House) NP analysis program. 
%The lower Z bound has been allowed to go negative.
%The fitting algorithm has been switched to one that works with bounds
...
ymin = min(yData)-2;
%zmin = min(zData(:))-2;
zmin = min(zData(:))-std(zData(:));

%% Set up fittype and options.
Lower = [zmin, 0, 0, 0, xmin, ymin, zmin];
